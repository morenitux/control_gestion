...
if (!$resulta) {
	...
	if ($cuantas_copias!=0) {
		...
		while ($n<$cuantas_copias) {
			$sql2 = new scg_DB;
			$sql2->query("select cve_depe,titulo_titu,nom_titu,car_titu from dependencia where cve_depe='$nom_ccp[$n]'");
			// echo "select cve_depe,titulo_titu,nom_titu,car_titu from dependencia where cve_depe='$nom_ccp[$n]'";
			if ($sql2->num_rows($sql2) > 0) {
				if ($sql2->next_record()) {
					$cve_depe_ccp		= $sql2->f("cve_depe");
					$titulo_titu_ccp	= $sql2->f("titulo_titu");
					$nom_titu_ccp		= $sql2->f("nom_titu");
					$car_titu_ccp		= $sql2->f("car_titu");
					$particula="[000";
					if ($titulo_titu_ccp!="") {
					...
					$particula.="$nom_titu_ccp.- $car_titu_ccp]";
