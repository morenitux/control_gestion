...
$sql = new scg_DB;
$sql->query("SELECT count(*) from documento where cve_dirigido='$id'");
...
if ($en_documento>0) {
		printf("<br>El destinatario que intenta borrar tiene documentos relacionados y no puede ser eliminado.");
	...
	if ($en_catalogo>0) {
		$resulta = $sql->query("DELETE from dirigido where clave='$id'");
		if (!$resulta) {
		...
		printf("<br>El destinatario que intenta borrar ya no existe.<br> Por favor revise los datos que seleccionó.");
		printf("<br><br><form action='admonCatalogo.php?sess=$sess&control_catalogos=9&origen=$origen' method='post'><input type='submit' value='Regresar'></form>");
	}
}
print("<br></center>\n<br>\n");
