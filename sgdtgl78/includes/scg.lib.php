...
class Scg_Session {
	...
	function Open_Session($sessid, $sessuserid) {
		...
		if ($sessid == "0") {
			...
			//verifica privilegios de acceso a modificaci�n de cat�logos y arma cadena
			...
			//verifica privilegios de acceso para el men� superior y arma cadena
			$query = "select upper(objeto) from privi_opcion where usuario='$this->sessuserid'";
			$sql->query($query);
			...
			if ($sql->num_rows($sql) > 0) {
				while($sql->next_record()) {
					...
					switch ($objeto) {
						...
						break;
						case "PBCONSURESP":
							$control_menu_superior=substr_replace ($control_menu_superior,"2",2,1);
						break;
						case "PBESTDOC":
...
function verify_login($username, $password) {
	...
	if ($numrows >= "1") { ///OJO OJO OJO aqui!!!!!!!!!!!!!!!!!!
		while($sql->next_record()) {
			if ($elusuarioestadeshabilitad=='S') { //en algun momento se puede poner un campo adicional en la tabla que permita saber si el usuario est� inhabilitado o no
				$verified["bit"] = 2;
			} else {
				$verified["bit"] = 1;
			}
	...
	if ($numrows == "1") {
		while($sql->next_record()) {
			...
			if ($ip_anterior == $ip) {
				$verified["bit"]	= 4; //ya estaba activa una sesi�n de la misma m�quina y ser� borrada la m�s antigua
			} else {
				$verified["bit"]	= 3; //ya est� activa en otra m�quina
				$time = time();
				if(($time - $hace_rato) >= $default->scg_time_forback) {
					$verified["bit"]	= 4; //no le hace que ya tuviera una sesi�n en otra m�quina, ser� cerrada...
				}
			}
		}
	}
