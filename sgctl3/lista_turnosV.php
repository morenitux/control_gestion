...
$query.=" WHERE documento.fol_orig=docsal.fol_orig and docsal.cve_depe=dependencia.cve_depe and docsal.fol_orig is not null and docsal.fec_comp<=now()  and cve_resp='N'";
switch ($tipo_reporte) {
...
if ($numero_renglones > 0) {
	...
	<table width=100% border=0 cellspacing=2 cellpadding=2>
		<tr bgcolor='#<? echo $bgcolor_titulos; ?>'>
			...
			<td align='center' class='chiquitoblanco'>No.Entrada</td>
	...
	while($sql->next_record()) {
		...
		echo "\t<td class='chiquito' align='left'>\n";
	  	...
	  	if (substr($control_menu_superior,1,1)!="2") {
			echo "&nbsp;<a href='listados.php?sess=$sess&control_botones_superior=1&variable=detalle_documento&tipo_reporte=$tipo_reporte&columna=$i&parametro=$parametro'><img src='$default->scg_graphics_url/cambio.gif' border='0' alt='Edici�n'></a>";
		}
	  	echo "\t</td>\n";
