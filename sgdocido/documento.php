...
function regform_Validator(f) {
	if (f.cual_submit.value=="GUARDAR") {
		...
		if (f.cve_docto.value.length < 1) {
			alert("El n�mero del documento no debe ser nulo \n(Escriba S/N cuando el documento no tenga n�mero)");
			f.cve_docto.focus();
			return(false);
		}
		...
		if (fechaElabValida) {
			...
			if (comparacion=='MAYOR') {
				alert ('La fecha de elaboraci�n no puede ser mayor a la fecha de hoy: '+'<? echo $fecha_now; ?>');
				f.fec_elab.focus();
				return(false);
			}
			...
			if (comparacion=='MAYOR') {
				alert ('La fecha de elaboraci�n no puede ser mayor a la fecha de acuse: '+f.fec_recep.value);
				f.fec_elab.focus();
				return(false);
			}
		} else {
		...
		if (f.txt_resum.value.length > largo_maximo) {
			alert("El tama�o m�ximo de este campo es de "+largo_maximo+" caracteres. Por favor reduzca su texto.");
			f.txt_resum.focus();
			return(false);
		}
...
<?php
	...
	if ($num_promotor > 0) {
		while($sql->next_record()) {
			...
			echo "var p".$cve_prom_js."Array =  new Array(\"('Seleccione el nombre1 -----------------------------','',true,true)\",\n";
			$i++;
			$sql2 = new scg_DB;
	...
	if ($num_remitente > 0) {
		while($sql->next_record()) {
			...
			echo "var r".$cve_prom_js."Array =  new Array(\"('Seleccione el nombre2 -----------------------------','',true,true)\",\n";
			$i++;
			$sql2 = new scg_DB;
 ...
 function despliegaFirmantesFromPromotor(inForm,selected) {
	if (selected!=0) {
		...
		if (inForm.cve_remite.selectedIndex>0 && inForm.cve_remite[inForm.cve_remite.selectedIndex].value!=inForm.cve_prom[inForm.cve_prom.selectedIndex].value) { //El remitente ya est� seleccionado y hay que combinar ambos listados
			...
			//primero el del promotor; el -2 es para no a�adir la raya y el "OTRO" en medio del nuevo listado
			for (var i=0; i < selectedArray.length-2; i++) {
				eval("inForm.firmante.options[i]=" + "new Option" + selectedArray[i]);
			}
			//despu�s el del remitente; el -1 es para no a�adir el "Seleccione el remitente---" en medio del nuevo listado
			for (var j=0; j < previousArray.length-1; j++) {
				eval("inForm.firmante.options[j+i]=" + "new Option" + previousArray[j+1]);
			}
		} else { //El remitente no ha sido seleccionado y se arma el listado de firmantes s�lo con base en el promotor.
 ...
 function despliegaFirmantesFromRemitente(inForm,selected) {
	if (selected!=0) {
		...
		if (inForm.cve_prom.selectedIndex>0 && inForm.cve_remite[inForm.cve_remite.selectedIndex].value!=inForm.cve_prom[inForm.cve_prom.selectedIndex].value) { //El promotor ya est� seleccionado y hay que combinar ambos listados
		...
		} else { //El promotor no ha sido seleccionado y se arma el listado de firmantes s�lo con base en el remitente.
			while (selectedArray.length < inForm.firmante.options.length) {
				inForm.firmante.options[(inForm.firmante.options.length - 1)] = null;
			}
			for (var i=0; i < selectedArray.length; i++) {
	  ...
	  <td width="25%" valign="<?php echo $alineacion_vertical; ?>" align="right"
	   <?
		if ($iovar==1) {
			...
			echo "<font class='chiquito'>N�mero de Entrada:</font>";
		} else {
			...
			echo "<font class='chiquito'>N�mero de Salida:</font>";
		}
	   ?>
	  </td>
	...
	<table width="100%" border="0" cellspacing="<? echo $espaciado; ?>" cellpadding="<? echo $espaciado; ?>">
	 <tr>
	  ...
	  <td width="15%" valign="<?php echo $alineacion_vertical; ?>" align="right"
		...
		?>
		<font class="chiquito">Elaboraci�n:</font>
	  </td>
	  <td width="35%" valign="<?php echo $alineacion_vertical; ?>" align="left">
	  ...
	  <select name="cve_dirigido">
	   <option value=0 selected>
	   	...
	   	switch ($iovar) {
	   	  	...
	   	  	break;
	   	  	case 2:
				echo "Seleccione el Area remitente ---------------------";
	   	  	break;
	   	}
		?>
	   </option>
	   <?php
	   		$sql = new scg_DB;
			$sql->query("select nombre, clave, baja from dirigido where baja is NULL order by nombre");
	  	...
	  	if (substr($control_menu_superior,1,1)=="1") {
			switch ($iovar) {
				case 1:
					...
					//echo 'jaladatos: '.$jaladatos.'si_hubo: '.$si_hubo;
				break;
				case 2:
	  ...
	  <select name="firmante" onChange='OtroFirmante(document.plantilla_documento,document.plantilla_documento.firmante.options[document.plantilla_documento.firmante.selectedIndex].value)'>
		<?php
			...
			if ($jaladatos=='sip' && $si_hubo=='sip') {
				...
				echo "<option value=0>Seleccione el nombre3 -----------------------------</option>\n";
			}
		?>
	 ...
	 <td width="88%" valign="<?php echo $alineacion_vertical; ?>" align="left">
	  <select name="cve_tipo">
	   <?php
			...
			$sql = new scg_DB;
			$sql->query("select cve_ins,instruccion  from instruccion where tipo='D' and baja is NULL order by instruccion");
			$num = $sql->num_rows($sql);
			if ($num > 0) {
				while($sql->next_record()) {
 ...
 <tr>
  <td width="100%" valign="top" align="center">
   <table width="100%" border="0" cellspacing="<? echo $espaciado; ?>" cellpadding="<? echo $espaciado; ?>">
	<tr>
	 <td width="40%" valign="<?php echo $alineacion_vertical; ?>" align="left">
	   <a href="javascript: newWindow('admin/admonCatalogo.php?sess=<? echo $sess; ?>&control_catalogos=3&origen=documento',600,400,'Principal','control_catalogos','ventana_auxiliar');"><font class="chiquito">Clasificaci�n:</font></a><br><font size=-1>(ctrl + mouse para selecci�n m�ltiple)</font><br>
	   <select multiple name="cve_tema[]" size="5">
		<?php
			$sql = new scg_DB;
			$sql->query("select cve_tema,topico from tema where tipo='T' and baja='F' order by topico");
			$num = $sql->num_rows($sql);
			if ($num > 0) {
				while($sql->next_record()) {
	 ...
	 <td width="60%" valign="<?php echo $alineacion_vertical; ?>" align="left"
		...
		?>
	   <font class="chiquito">Asunto:</font><br><font size=-1>(s�ntesis)</font><br>
