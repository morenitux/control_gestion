...
if ($discriminador=="recepcion_directa_de_buzon") {
	...
	$query="update docsal set cve_docto='$cve_docto_resp', txt_resp='$sintesis', cve_resp='$cve_resp', viable='$viable', califresp='$califresp', fec_regi='$fecha_now',";
} else {
	$query="update docsal set cve_docto='$cve_docto', txt_resp='$txt_resp', cve_resp='$cve_resp', viable='$viable', califresp='$califresp', fec_regi='$fecha_now',";
}
if ($fec_elab!="") {
	$query.="fec_elab='$fec_elab',";
}
if ($fec_recdp!="") {
	$query.="fec_recdp='$fec_recdp',";
}
if ($fec_comp!="") {
	$query.="fec_comp='$fec_comp',";
}
if ($fec_notifica!="") {
	$query.="fec_notifica='$fec_notifica',";
}
if ($fec_conclu!="") {
	$query.="fec_conclu='$fec_conclu',";
}
if ($viene_de=="buzon_respuestas") {
	$query.="fec_recdp='$fecha_now',";
}
if ($fec_conclu=="S") {
...
if ($sql->num_rows($sql) > 0) {
	...
	//FIN DE LA REVISION
	if ($base_datos && $transmitir=='1') { //SI SE ENCONTR� EN depe_buzon
		...
		if ($sql->num_rows($sql) > 0) {
			$insertar_buzon = "INSERT into resp_buzon (fol_orig, conse, fec_salid, remite, cve_docto,";
			if ($fec_elab!="") {
				$insertar_buzon.="fec_elab,";
			}
			$insertar_buzon.="sintesis, base_datos, prop_base, folio_remite, conse_remite, plazo,";

			if ($fec_notifica!="") {
				$insertar_buzon.="fec_notifica,";
			 }

			$insertar_buzon.="cve_resp, viable, etapas,";

			if ($fec_conclu!="") { $insertar_buzon.="fec_conclu,"; }

			$insertar_buzon.="califresp";

			if ($fec_comp!="") { $insertar_buzon.=",fec_compro"; }

			$insertar_buzon.= ") values ('$fol_refe', '$conse_refe' , '$fecha_now', '$licencia', '$cve_docto',";

			if ($fec_elab!="") {
				$insertar_buzon.= "'$fec_elab',";
			}
			$insertar_buzon.= "'$txt_resp', '$base_datos_origen', '$prop_base_origen', '$fol_parametro', '$conse_parametro', '$plazo',";

			if ($fec_notifica!="") {
				$insertar_buzon.="'$fec_notifica',";
			}
			$insertar_buzon.="'$cve_resp', '$viable', '$texto_etapas',";
		...
		$resulta = $sql->query("update docsal set transresp='T', fec_envioresp='$fecha_now' where fol_orig='$fol_parametro' and conse='$conse_parametro'");
		if (!$resulta) {
			printf("<br> Error en el query o con la base de datos !\n");
		}
	}
