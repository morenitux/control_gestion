...
$titulo_reporte="Control de entrega (COPIAS)";
$query="select documento.fol_orig,nom_ccp,to_char(documento.fec_regi,'dd/mm/yyyy') as fec_regi,to_char(documento.fec_elab,'dd/mm/yyyy') as fec_elab,documento.cve_docto,documento.firmante,documento.cve_remite,docsal.coment as txt_resum,documento.nom_suje from documento,docsal,salccp where documento.fol_orig!='' and documento.fol_orig=docsal.fol_orig and docsal.fol_orig=salccp.fol_orig and docsal.conse=salccp.conse and ";
switch ($fechas) {
	case "1": //Hoy
...
while($sql->next_record()) {
	...
	//ccedillo-error reportado 27-12-2002     echo $fol_orig." - ".$nom_ccp."<br>";
	...
	for ($y=0; $y<$num_cachos; $y++) {
		if (substr($arreglo_copias[$y],0,3)=='999') {
			...
			if ($a1>$a2) {
				...
				//ccedillo-error reportado 27-12-2002 echo $arreglo_copias[$y]."-";
				$controlador++;
			}
		}
	}
	//ccedillo-error reportado 27-12-2002 echo "<br>";
...
//ccedillo-error reportado 27-12-2002 echo "ANTES DE SORTEAR<br>";
for ($i=0; $i<$total_global; $i++) {
	//ccedillo-error reportado 27-12-2002 echo ($i+1).".- ".$campo[2][$i]." - ".$campo[1][$i]."<br>";
}
...
//ccedillo-error reportado 27-12-2002 echo "<br>DESPUES DE SORTEAR<br>";
for ($i=0; $i<$total_global; $i++) {
	//ccedillo-error reportado 27-12-2002 echo ($i+1).".- ".$campo[2][$i]." - ".$campo[1][$i]."<br>";
	//." - ".$campo[4][$i]."-".$campo[5][$i]."-".$campo[6][$i]."-".$campo[7][$i]."-".$campo[8][$i]."-".$campo[9][$i]."<br>";
...
$pdf->Text($x+90,$y+185,"Total: ".($registros-1)." copia(s)");
$registros=1;
$renglon=0;
}
...
$pdf->Text($x+54,$y+20,"PROCEDENCIA");
$pdf->Text($x+94,$y+20,"FIRMADO POR");
