...
$sql = new scg_DB;
$sql->query("SELECT count(*) from docsal where cve_turn='$id'");
...
if ($en_documento>0) {
		printf("<br>El turnador que intenta borrar tiene turnos y/o documentos relacionados y no puede ser eliminado.");
	...
	$sql->query("SELECT count(*) from turnador where cve_turn='$id'");
	if ($sql->num_rows($sql)>0) {
		while($sql->next_record()) {
			$en_catalogo = $sql->f("0");
		}
	...
	if ($en_catalogo>0) {
		$resulta = $sql->query("DELETE from turnador where cve_turn='$id'");
		if (!$resulta) {
		...
		printf("<br>El turnador que intenta borrar ya no existe.<br> Por favor revise los datos que seleccionó.");
		printf("<br><br><form action='admonCatalogo.php?sess=$sess&control_catalogos=6' method='post'><input type='submit' value='Regresar'></form>");
	}
}
print("<br></center>\n<br>\n");
?>