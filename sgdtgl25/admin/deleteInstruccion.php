...
$sql = new scg_DB;
$sql->query("SELECT count(*) from docsal where cve_ins='$id' union SELECT count(*) from documento where cve_tipo='$id'");
...
if ($en_documento>0) {
	...
	if ($en_catalogo>0) {
		$resulta = $sql->query("DELETE from instruccion where cve_ins='$id'");
		if (!$resulta) {
		...
		print("<br>El registro que intenta borrar ya no existe.<br> Por favor revise los datos que seleccionó.");
		print("<br><br><form action='admonCatalogo.php?sess=$sess&control_catalogos=");
		switch ($tipo) {
			case "I":
				print("7");
